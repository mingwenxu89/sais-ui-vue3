<template>
  <Dialog v-model="dialogVisible" :title="dialogTitle" max-height="500px" scroll>
    <el-form
      ...
      label-width="160px"
    >
      <el-form-item label="Client ID" prop="secret">
        <el-input v-model="formData.clientId" placeholder="Enter client ID" />
      </el-form-item>
      <el-form-item label="Client Secret" prop="secret">
        <el-input v-model="formData.secret" placeholder="Enter客户端密钥" />
      </el-form-item>
      <el-form-item label="App Name" prop="name">
        <el-input v-model="formData.name" placeholder="Enter应用名" />
      </el-form-item>
      <el-form-item label="应用图标">
        <UploadImg v-model="formData.logo" :limit="1" />
      </el-form-item>
      <el-form-item label="应用描述">
        <el-input v-model="formData.description" placeholder="Enter应用名" type="textarea" />
      </el-form-item>
      ...
      <el-form-item label="Access Token Validity (s)" prop="accessTokenValiditySeconds">
        <el-input-number v-model="formData.accessTokenValiditySeconds" placeholder="单位：秒" />
      </el-form-item>
      <el-form-item label="Refresh Token Validity (s)" prop="refreshTokenValiditySeconds">
        <el-input-number v-model="formData.refreshTokenValiditySeconds" placeholder="单位：秒" />
      </el-form-item>
      <el-form-item label="Grant Types" prop="authorizedGrantTypes">
        <el-select
          v-model="formData.authorizedGrantTypes"
          filterable
          multiple
          placeholder="Enter授权类型"
          style="width: 500px"
        >
          <el-option
      ...
      <el-form-item label="Scopes" prop="scopes">
        <el-select
          ...
          multiple
          allow-create
          placeholder="Enter授权范围"
          style="width: 500px"
        >
          <el-option v-for="scope in formData.scopes" :key="scope" :label="scope" :value="scope" />
        </el-select>
      </el-form-item>
      <el-form-item label="Auto Approve Scopes" prop="autoApproveScopes">
        <el-select
      ...
      <el-form-item label="Redirect URIs" prop="redirectUris">
        <el-select
      ...
      <el-form-item label="Resources" prop="resourceIds">
        <el-select
      ...
      <el-form-item label="Additional Info" prop="additionalInformation">
        <el-input
          v-model="formData.additionalInformation"
          placeholder="Enter additional info (JSON format)"
          type="textarea"
        />
      </el-form-item>
    </el-form>
...
const formRules = reactive({
  ...
  secret: [{ required: true, message: 'Client secret cannot be empty', trigger: 'blur' }],
  name: [{ required: true, message: 'App name cannot be empty', trigger: 'blur' }],
  logo: [{ required: true, message: 'App icon cannot be empty', trigger: 'blur' }],
  status: [{ required: true, message: 'Status cannot be empty', trigger: 'blur' }],
  accessTokenValiditySeconds: [
    { required: true, message: 'Access token validity cannot be empty', trigger: 'blur' }
  ],
  refreshTokenValiditySeconds: [
    { required: true, message: '刷新令牌的有效期不能为空', trigger: 'blur' }
  ],
  redirectUris: [{ required: true, message: 'Redirect URI cannot be empty', trigger: 'blur' }],
